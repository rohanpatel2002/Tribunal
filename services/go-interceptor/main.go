...
func main() {
	...
	if enterpriseAPIKey != "" {
		...
		mux.HandleFunc("/api/v1/audit/repositories", corsWrapper(RequireAuth(enterpriseAPIKey, h.getRepositoryAnalysisCountsHandler)))
		...
		mux.HandleFunc("/api/v1/audit/repositories", corsWrapper(h.getRepositoryAnalysisCountsHandler))
